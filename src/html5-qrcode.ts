...
export class Html5Qrcode {
  ...
  public stop(): Promise<void> {
    ...
    const removeQrRegion = () => {
      ...
      let childElement = document.getElementById(
        Constants.SHADED_REGION_ELEMENT_ID
      );
      if (childElement) {
  ...
  public scanFile(
    imageFile: File,
    /* default=true */ showImage?: boolean
  ): Promise<string> {
    return this.scanFileV2(imageFile, showImage).then(
      (html5qrcodeResult) => html5qrcodeResult.decodedText
    );
  }
  ...
  public applyVideoConstraints(
    ...
    if (!videoConstaints) {
      ...
      !VideoConstraintsUtil.isMediaStreamConstraintsValid(
        videoConstaints,
        this.logger
      )
    ) {
  ...
  private static getCamerasFromMediaStreamTrack(): Promise<
    Array<CameraDevice>
  > {
    return new Promise((resolve, _) => {
  ...
  private getSupportedFormats(
    ...
    if (!Array.isArray(configOrVerbosityFlag.formatsToSupport)) {
      throw (
        "configOrVerbosityFlag.formatsToSupport should be undefined " +
        "or an array."
      );
    }
  ...
  private validateQrboxSize(
    ...
    const correctWidthBasedOnRootElementSize = (configWidth: number) => {
      if (configWidth > viewfinderWidth) {
        this.logger.warn(
          "`qrbox.width` or `qrbox` is larger than the" +
  ...
  private validateQrboxConfig(
    ...
    if (qrboxSize.width === undefined || qrboxSize.height === undefined) {
      throw (
        "Invalid instance of QrDimensions passed for " +
  ...
  private setupUi(
    ...
    if (qrDimensions.height > viewfinderHeight) {
      this.logger.warn(
        "[Html5Qrcode] config.qrbox has height that is" +
  ...
  private createScannerPausedUiElement(rootElement: HTMLElement) {
    ...
    scannerPausedUiElement.innerText = "";
    ...
    scannerPausedUiElement.style.background = "transparent";
  ...
  private scanContext(
    ...
    return this.qrcode
      ...
      .catch((error) => {
        ...
        qrCodeErrorCallback(
          errorMessage,
          Html5QrcodeErrorFactory.createFrom(errorMessage)
        );
        return false;
      });
  }
    ...
    this.scanContext(qrCodeSuccessCallback, qrCodeErrorCallback)
      .then((isSuccessfull) => {
        ...
        if (!isSuccessfull && internalConfig.disableFlip !== true) {
          ...
          this.scanContext(qrCodeSuccessCallback, qrCodeErrorCallback).finally(
            () => {
              triggerNextScan();
            }
          );
  ...
  private getShadedRegionBounds(
    ...
    if (qrboxSize.width > width || qrboxSize.height > height) {
      throw (
        "'config.qrbox' dimensions should not be greater than the " +
  ...
  private insertShaderBorders(
    ...
    elem.style.backgroundColor = "#00000";
